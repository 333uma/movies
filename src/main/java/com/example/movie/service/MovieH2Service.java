...
public class MovieH2Service implements MovieRepository{
    ...
    public Movie getMovieById(int movieId){
        try{
            Movie m = db.queryForObject("SELECT * FROM MOVIELIST WHERE movieId = ?", new MovieRowMapper(), movieId);
            return m;
        }
        catch(Exception e){
    ...
    public Movie addMovie(Movie movie){
        ...
        Movie m = db.queryForObject("SELECT * FROM MOVIELIST WHERE movieName = ? and leadActor = ?", new MovieRowMapper(), movie.getMovieName(), movie.getLeadActor());
        return m;
    }
